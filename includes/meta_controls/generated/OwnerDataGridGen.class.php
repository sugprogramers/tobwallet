<?php
	 ...
	 * listing a collection of Owner objects.  It includes
	 ...
	 *
	 */
	class OwnerDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Owner datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Owner object, or it can be any QQNode extending from QQN::Owner().
		 *
		 ...
		 * sorting using two fields from the Owner object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Owner
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdOwner=<?=urlencode($_ITEM->IdOwner)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdOwner)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdOwner, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Owner::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Owner, given the clauses above
			$this->DataSource = Owner::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Owner-based QQNode.
		 * It will also verify that it is a proper Owner-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'owner') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "owner".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdOwner': return QQN::Owner()->IdOwner;
				case 'IdUser': return QQN::Owner()->IdUser;
				case 'IdUserObject': return QQN::Owner()->IdUserObject;
				default: throw new QCallerException('Simple Property not found in OwnerDataGrid content: ' . $mixContent);
