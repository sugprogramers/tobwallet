<?php
	 ...
	 * listing a collection of Queueemail objects.  It includes
	 ...
	 *
	 */
	class QueueemailDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Queueemail datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Queueemail object, or it can be any QQNode extending from QQN::Queueemail().
		 *
		 ...
		 * sorting using two fields from the Queueemail object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Queueemail
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdQueueEmail=<?=urlencode($_ITEM->IdQueueEmail)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdQueueEmail)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdQueueEmail, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Queueemail::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Queueemail, given the clauses above
			$this->DataSource = Queueemail::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Queueemail-based QQNode.
		 * It will also verify that it is a proper Queueemail-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'queueemail') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "queueemail".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdQueueEmail': return QQN::Queueemail()->IdQueueEmail;
				case 'To': return QQN::Queueemail()->To;
				case 'Subject': return QQN::Queueemail()->Subject;
				case 'Body': return QQN::Queueemail()->Body;
				case 'Status': return QQN::Queueemail()->Status;
				case 'Log': return QQN::Queueemail()->Log;
				case 'IdUser': return QQN::Queueemail()->IdUser;
				case 'CreateDateTime': return QQN::Queueemail()->CreateDateTime;
				default: throw new QCallerException('Simple Property not found in QueueemailDataGrid content: ' . $mixContent);
