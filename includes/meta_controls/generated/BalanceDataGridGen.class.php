<?php
	 ...
	 * listing a collection of Balance objects.  It includes
	 ...
	 *
	 */
	class BalanceDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Balance datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Balance object, or it can be any QQNode extending from QQN::Balance().
		 *
		 ...
		 * sorting using two fields from the Balance object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Balance
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdBalance=<?=urlencode($_ITEM->IdBalance)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdBalance)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdBalance, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Balance::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Balance, given the clauses above
			$this->DataSource = Balance::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Balance-based QQNode.
		 * It will also verify that it is a proper Balance-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'balance') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "balance".');
