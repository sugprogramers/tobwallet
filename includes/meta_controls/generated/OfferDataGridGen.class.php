<?php
	...
	class OfferDataGridGen extends QDataGrid {
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdOffer=<?=urlencode($_ITEM->IdOffer)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdOffer)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdOffer, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Offer::QueryCount($objConditions);
			}
			...
			$this->DataSource = Offer::QueryArray($objConditions, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'IdOffer': return QQN::Offer()->IdOffer;
				case 'Description': return QQN::Offer()->Description;
				case 'OfferedCoins': return QQN::Offer()->OfferedCoins;
				case 'DateFrom': return QQN::Offer()->DateFrom;
				case 'DateTo': return QQN::Offer()->DateTo;
				case 'IdRestaurant': return QQN::Offer()->IdRestaurant;
				case 'IdRestaurantObject': return QQN::Offer()->IdRestaurantObject;
