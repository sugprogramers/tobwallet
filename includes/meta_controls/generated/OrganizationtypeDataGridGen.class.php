<?php
	 ...
	 * listing a collection of Organizationtype objects.  It includes
	 ...
	 *
	 */
	class OrganizationtypeDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Organizationtype datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Organizationtype object, or it can be any QQNode extending from QQN::Organizationtype().
		 *
		 ...
		 * sorting using two fields from the Organizationtype object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Organizationtype
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdOrganizationType=<?=urlencode($_ITEM->IdOrganizationType)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdOrganizationType)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdOrganizationType, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Organizationtype::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Organizationtype, given the clauses above
			$this->DataSource = Organizationtype::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Organizationtype-based QQNode.
		 * It will also verify that it is a proper Organizationtype-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'organizationtype') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "organizationtype".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdOrganizationType': return QQN::Organizationtype()->IdOrganizationType;
				case 'Name': return QQN::Organizationtype()->Name;
				case 'Description': return QQN::Organizationtype()->Description;
				default: throw new QCallerException('Simple Property not found in OrganizationtypeDataGrid content: ' . $mixContent);
