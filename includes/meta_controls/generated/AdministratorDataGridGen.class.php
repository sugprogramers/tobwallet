<?php
	 ...
	 * listing a collection of Administrator objects.  It includes
	 ...
	 *
	 */
	class AdministratorDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Administrator datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Administrator object, or it can be any QQNode extending from QQN::Administrator().
		 *
		 ...
		 * sorting using two fields from the Administrator object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Administrator
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdAdministrator=<?=urlencode($_ITEM->IdAdministrator)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdAdministrator)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdAdministrator, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Administrator::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Administrator, given the clauses above
			$this->DataSource = Administrator::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Administrator-based QQNode.
		 * It will also verify that it is a proper Administrator-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'administrator') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "administrator".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdAdministrator': return QQN::Administrator()->IdAdministrator;
				case 'Email': return QQN::Administrator()->Email;
				case 'Password': return QQN::Administrator()->Password;
				case 'FirstName': return QQN::Administrator()->FirstName;
				case 'LastName': return QQN::Administrator()->LastName;
				case 'Address': return QQN::Administrator()->Address;
				case 'Phone': return QQN::Administrator()->Phone;
				default: throw new QCallerException('Simple Property not found in AdministratorDataGrid content: ' . $mixContent);
