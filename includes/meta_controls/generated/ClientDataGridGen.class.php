<?php
	 ...
	 * listing a collection of Client objects.  It includes
	 ...
	 *
	 */
	class ClientDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Client datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Client object, or it can be any QQNode extending from QQN::Client().
		 *
		 ...
		 * sorting using two fields from the Client object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Client
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdClient=<?=urlencode($_ITEM->IdClient)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdClient)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdClient, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Client::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Client, given the clauses above
			$this->DataSource = Client::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Client-based QQNode.
		 * It will also verify that it is a proper Client-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'client') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "client".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdClient': return QQN::Client()->IdClient;
				case 'IdUser': return QQN::Client()->IdUser;
				case 'IdUserObject': return QQN::Client()->IdUserObject;
				default: throw new QCallerException('Simple Property not found in ClientDataGrid content: ' . $mixContent);
