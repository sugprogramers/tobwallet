<?php
	 ...
	 * listing a collection of Versionkcoin objects.  It includes
	 ...
	 *
	 */
	class VersionkcoinDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Versionkcoin datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Versionkcoin object, or it can be any QQNode extending from QQN::Versionkcoin().
		 *
		 ...
		 * sorting using two fields from the Versionkcoin object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Versionkcoin
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdVersionKcoin=<?=urlencode($_ITEM->IdVersionKcoin)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdVersionKcoin)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdVersionKcoin, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Versionkcoin::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Versionkcoin, given the clauses above
			$this->DataSource = Versionkcoin::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Versionkcoin-based QQNode.
		 * It will also verify that it is a proper Versionkcoin-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'versionkcoin') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "versionkcoin".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdVersionKcoin': return QQN::Versionkcoin()->IdVersionKcoin;
				case 'Name': return QQN::Versionkcoin()->Name;
				case 'Path': return QQN::Versionkcoin()->Path;
				default: throw new QCallerException('Simple Property not found in VersionkcoinDataGrid content: ' . $mixContent);
