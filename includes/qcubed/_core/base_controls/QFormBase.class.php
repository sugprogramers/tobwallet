<?php
	...
	abstract class QFormBase extends QBaseClass {
		...
		protected $strCustomAttributeArray = null;		
                ...
                public function simulateFormInRam(){ 
                    $this->objControlArray=array(); 
                    $this->objGroupingArray=array();
                    $this->intFormStatus = QForm::FormStatusRenderBegun;
...
//			$strToReturn = sprintf('control%s', $this->intNextControlId);
		...
		public static function Run($strFormId, $strAlternateHtmlFile = null) {
			...
			if (array_key_exists('Qform__FormId', $_POST) && ($_POST['Qform__FormId'] == $strFormId) && array_key_exists('Qform__FormState', $_POST)) {
				...
				if (!$objClass){ /*throw new QInvalidFormStateException($strFormId);*/ QApplication::Redirect(__SUBDIRECTORY__ . "/login");}
			}
...
/*				$strControlId = $objParentObject->ControlId;
		...
		public function RenderBegin($blnDisplayOutput = true) {
			...
			// Setup Rendered HTML
                        ...
                        $action =  htmlentities(QApplication::$RequestUri);
                        if(trim($this->strAction)!='')
                         $action =  $this->strAction;
                        
			$strToReturn .= sprintf('<form method="post" id="%s" action="%s"%s>', $this->strFormId, $action, $strFormAttributes);
			$strToReturn .= "\r\n";
		...
		public function RenderEnd($blnDisplayOutput = true) {
			...
			//$strEndScript = sprintf('<script type="text/javascript">$j(document).ready(function() { %s; });      $j("#content1").fadeTo(500,0.8,function(){  $j("#content1").fadeTo(500,1);});  </script>', $strEndScript);
