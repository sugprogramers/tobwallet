...
{
        public $VisiblechkSelectAll=true; 
	...
	protected $strCheckboxCallbackFunc = null;
        ...
        public $strNewJavaScript = "";
        public $strJSActionInCheckBoxes = "";
		...
		if(null === $this->chkSelectAll) {
			...
			//of these on a single form
                        if($this->strNewJavaScript == ""){
                            ...
                            $strJavascript = sprintf("var datagrid = document.getElementById('%s');var selectAll = document.getElementById('%s');var childInputs = datagrid.getElementsByTagName('input');for(var i = 0; i < childInputs.length; i++){var subid = childInputs[i].id.substring(%s, 0);if(subid == '%s')childInputs[i].checked = selectAll.checked;}",$this->objDataGrid->ControlId, $this->chkSelectAll->ControlId, $strControlIdStartLen, $strControlIdStart);
                        }else{
                            $strJavascript = sprintf($this->strNewJavaScript,$this->objDataGrid->ControlId, $this->chkSelectAll->ControlId, $strControlIdStartLen, $strControlIdStart);
                        }
			$this->chkSelectAll->AddAction(new QClickEvent(), new QJavaScriptAction($strJavascript));
		}
		if($this->VisiblechkSelectAll == true)
	...
	public function chkSelected_Render($_ITEM) {
		...
		if (!$chkSelected) {
			$chkSelected = new QCheckBox($this->objDataGrid, $strControlId);
                        if($this->strJSActionInCheckBoxes != "")
                            $chkSelected->AddAction(new QClickEvent(), new QJavaScriptAction($this->strJSActionInCheckBoxes));
                        if(isset($this->initValues['a'.$intId])) $chkSelected->Checked=true;
	...
	{       $colIndex = $this->GetColIndex();
		...
		$checkbox = $this->objDataGrid->GetChildControl($controlId);
                if(null === $checkbox && $value==true){
                    $this->initValues['a'.$itemId]=true;
                    return ;
                }
                if(isset($this->initValues['a'.$itemId])) unset($this->initValues['a'.$itemId]) ;//agregue
		$checkbox->Checked = $value;
	}
