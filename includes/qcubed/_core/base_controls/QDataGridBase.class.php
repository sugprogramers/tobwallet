...
class QDataGridRow extends QControl {
    ...
    public function GetHtml($strColumnsHtml) {
        ...
        //$ex = explode('row', $this->strControlId);
        //onClick="javascript:selectRowTable(\''.$this->ControlId.'\',\''.$ex[0].'\' );"   
...
abstract class QDataGridBase extends QPaginatedControl {
    ...
    public $titlePanel = "DataGrid";
    public $selectRowJavascript = true;
    // APPEARANCE
    ...
    public function __construct($objParentObject, $strControlId = null) {
        ...
        // Labels
        $this->strLabelForNoneFound = QApplication::Translate('No %s found '); //'<b>Results:</b> No %s found.'
        $this->strLabelForOneFound = QApplication::Translate('Results: 1 %s found.');
        $this->strLabelForMultipleFound = QApplication::Translate('Results: %s %s found.');
        $this->strLabelForPaginated = QApplication::Translate('%s&nbsp;%s - %s&nbsp;of&nbsp;%s'); //'<b>Results:</b>&nbsp;Viewing&nbsp;%s&nbsp;%s-%s&nbsp;of&nbsp;%s.'
    ...
    protected function GetHeaderSortedHtml(QDataGridColumn $objColumn) {
        ...
        if ($this->intSortDirection == 0)
            $strToReturn .= sprintf(' <img src="%s/sort_asc.png" width="12" height="12" border="0" alt="Sorted" />', __VIRTUAL_DIRECTORY__ . __IMAGE_ASSETS__);
        else
            $strToReturn .= sprintf(' <img src="%s/sort_desc.png" width="12" height="12" border="0" alt="Reverse Sorted" />', __VIRTUAL_DIRECTORY__ . __IMAGE_ASSETS__);
    ...
    protected function GetHeaderRowHtml() {
        ...
        $ArrayColumns = $this->objColumnArray;

        if ($this->objColumnArray) {


            $lastIndex = count($this->objColumnArray)-1;
            foreach ($this->objColumnArray as $objColumn) {
                $idHover = $this->ControlId . "_colhover_" . $intColumnIndex;
                $idMenu = $this->ControlId . "_colmenu_" . $intColumnIndex;

                /* $stringcolums ="";
                  $countC = 0;
                  foreach ($ArrayColumns as $arryc)
                  {
                  $nameColumn="column".$countC;
                  if(trim($arryc->NameGeneral)!='')
                  $nameColumn= strip_tags($arryc->NameGeneral);
                  if($countC < 2)
                  $stringcolums .= sprintf("<li><a href=\"#\"><input type='checkbox' id='%s' checked='true'  onclick=\"javascript:show_hide_column('%s', '%s','%s')\" > %s </a></li>",
                  $this->ControlId . "_colHCheck_" . $countC .'_'.$intColumnIndex,
                  $countC ,
                  $this->ControlId . "_colHCheck_" . $countC .'_'.$intColumnIndex,
                  $this->ControlId ,
                  $nameColumn);
                  ...
                  <li class="nivel1">
                  <a href="#"> <div class="menuTableColums"></div> Columns</a>
                  <ul>
                  '.$stringcolums.'
                  </ul>
                  </li>
                ...
                /* $newmenu ='<div class="menuTable" id="'.$idMenu.'" >
                  <ul>
                  <li class="nivel1">
                  <a href="#"  > <div class="menuTableAsc"></div> Sort Ascending </a>
                  </li>
                  <li class="nivel1">
                  <a href="#" > <div class="menuTableDesc"></div> Sort Descending</a>
                  </li>
                ...
                if ($objColumn->OrderByClause) {
                    ...
                    //$strToReturn .= sprintf("    <th %s   ondblclick=\"javascript:selectColumnTable($intColumnIndex,'".$this->ControlId."');\"    onmouseover=\"javascript:showDivTable('$idHover');\" onmouseout=\"javascript:hideDivTable('$idHover');\"  >  <div class='div1Headertable' >  <a id=\"%s\" href=\"%s\" %s%s>%s</a>   <div id=\"$idHover\" class=\"hoverHeadertable\"   onmouseover=\"javascript:showDivTable('$idMenu');\" onmouseout=\"javascript:hideDivTable('$idMenu');\" > $newmenu </div> </div>  </th>\r\n",
                    $strToReturn .= sprintf("    <th %s   ondblclick=\"javascript:selectColumnTable($intColumnIndex,'" . $this->ControlId . "');\"   >  <a id=\"%s\" href=\"%s\" %s%s>%s</a>   </th>\r\n", $this->objHeaderRowStyle->GetAttributes(), $this->ControlId . "_col_" . $this->strActionParameter, QApplication::$RequestUri, $this->prxDatagridSorting->RenderAsEvents($this->strActionParameter, true, $this->ControlId . "_col_" . $this->strActionParameter, false), $this->objHeaderLinkStyle->GetAttributes(), $strName);
                } else {
                   
                    if (!property_exists($objColumn, 'VisiblechkSelectAll')) {
                        if ($intColumnIndex == $lastIndex) {
                            $strToReturn .= sprintf("    <th %s ondblclick=\"javascript:selectColumnTable($intColumnIndex,'" . $this->ControlId . "');\"  >  <div class='div1Headertable'>   %s   </div></th>\r\n", $this->objHeaderRowStyle->GetAttributes(), $objColumn->Name);
                        } else {
                            $strToReturn .= sprintf("    <th %s ondblclick=\"javascript:selectColumnTable($intColumnIndex,'" . $this->ControlId . "');\"  >  <div class='div1Headertable'>   %s   </div></th>\r\n", $this->objHeaderRowStyle->GetAttributes(), $objColumn->Name);
                        }
                    } else {
                        ...
                        if ($intColumnIndex == $lastIndex) {
                            $strToReturn .= sprintf("    <th ondblclick=\"javascript:selectColumnTable($intColumnIndex,'" . $this->ControlId . "');\"     %s >  <div class='div1Headertable'>   %s   </div>  </th>\r\n", $this->objHeaderRowStyle->GetAttributes(), $objColumn->Name);
                        } else {
                            $strToReturn .= sprintf("    <th ondblclick=\"javascript:selectColumnTable($intColumnIndex,'" . $this->ControlId . "');\"     %s >  <div class='div1Headertable'>   %s   </div>  </th>\r\n", $this->objHeaderRowStyle->GetAttributes(), $objColumn->Name);
                        }
                    }
    ...
    protected function GetDataGridRowHtml($objObject) {
        ...
        if ($numEvents > 0) {
            ...
            if ($this->selectRowJavascript) {
                $strToReturn = sprintf('<tr id="%s"  %s  onClick="javascript:selectRowTable(\''.$strTrId.'\',\''.$this->strControlId.'\' );"     >%s</tr>', $strTrId, $objStyle->GetAttributes(), $strColumnsHtml);
            } else {
    ...
    protected function GetControlHtml() {
        ...
        if ($this->objPaginator)
            $strToReturn .= "<caption>\r\n" . $this->GetPaginatorRowHtml($this->objPaginator) . "{{MENU}}</caption>\r\n";
        ...
        if ($this->showPanel == true) {
            ...
            $stringcolums = "";
            $countC = 0;
            foreach ($this->objColumnArray as $arryc) {
                $nameColumn = "Column" . $countC;
                if (trim($arryc->NameGeneral) != '')
                    $nameColumn = strip_tags($arryc->NameGeneral);

                //$stringcolums .= sprintf("<li><a href=\"#\"><input  type='checkbox' id='%s' checked='true'  onclick=\"javascript:show_hide_column('%s', '%s','%s');\" > %s </a></li>", $this->ControlId . "_colHeaderCheck_" . $countC, $countC, $this->ControlId . "_colHeaderCheck_" . $countC, $this->ControlId, $nameColumn);
                $stringcolums .= sprintf("<li><label><input type='checkbox' id='%s' checked='true'  onclick=\"javascript:show_hide_column('%s', '%s','%s');\"  > %s </label></li>  ", $this->ControlId . "_colHeaderCheck_" . $countC, $countC, $this->ControlId . "_colHeaderCheck_" . $countC, $this->ControlId, $nameColumn);
                $countC ++;
            }
            
            
$buttonrefresh  =  $this->btnRefresh_Create($this->strControlId.'rfh');
$idfinalwait ='';
if(isset($this->objWaitIcon) && $this->objWaitIcon!='' && $this->objWaitIcon!='default'){
$idfinalwait = $this->objWaitIcon->ControlId;
}
$menuGrid = '<span><div class="fixed-table-toolbar">
<div class="columns columns-right btn-group pull-right" style="margin-top:0px;">
<div class="keep-open btn-group" title="Columns">
<button class="btn btn-default btn-outline" type="button" name="refresh" title="Refresh" onClick="javascript:javarefresh(\''.$this->objForm->FormId.'\',\''.$this->strControlId.'rfh\' ,\''.$idfinalwait.'\');"
><i class="glyphicon wb-refresh"></i></button>
<button type="button" class="btn btn-default btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
<i class="glyphicon wb-list-bulleted">
</i> 
...
</ul></div></div></div></span>'.$buttonrefresh->Render(false);
            
$strToReturn = str_replace('{{MENU}}', $menuGrid, $strToReturn);
            
            
            

            /* $idmenu= 'MenuTable'.$this->strControlId;
              $newmenu ='<div class="menuTable" id="'.$idmenu.'" >
              <ul>
              <li class="nivel1">
              <a href="#"> <div class="menuTableColums"></div> Columns</a>
            ...
            /* $strToReturn = $buttonrefresh->Render(false).'
              <div>
              <div   style="width:100%;height: 36px;background-color:#157FCC;text-align: left;color:#fff;" >

              <div id="'.$this->strControlId.'icongrid"  class="IconGrid" style="margin:8px 5px 10px 6px;float:left;height:16px;width:16px;" >
              </div>
              <div style="float:left;margin:10px 0px 10px 0px;font-size: 12.5px;font-weight: bold;height:16px;width:16px;">
              '.$this->titlePanel.'
              </div>



              <div id="'.$this->strControlId.'icon1"  class="CollapsibleVerticalNone" style="margin:10px 7px 10px 7px;float:right;height:16px;width:16px;" onClick="javascript:panelCollapsibleVertical(\''.$this->strControlId.'content1\',\''.$this->strControlId.'icon1\');">

              </div>



              <div id="'.$this->strControlId.'icon2"  class="ColumnsVertical" style="margin:10px 7px 10px 7px;float:right;height:16px;width:16px;" onmouseover="javascript:showDivTable(\''.$idmenu.'\');" onmouseout="javascript:hideDivTable(\''.$idmenu.'\');">
              '.$newmenu.'
              </div>


              <div id="'.$this->strControlId.'icon3"  class="RefreshVertical" style="margin:10px 7px 10px 7px;float:right;height:16px;width:16px;" onClick="javascript:javarefresh(\''.$this->objForm->FormId.'\',\''.$this->strControlId.'rfh\' ,\''.$idfinalwait.'\');">

              </div>

              <div id="'.$this->strControlId.'icon4"  class="SearchVertical" style="margin:10px 7px 10px 7px;float:right;height:16px;width:16px;"  onclick="window.find();" >

              </div>

              <div id="'.$this->strControlId.'icon5"  class="HelpVertical" style="margin:10px 7px 10px 7px;float:right;height:16px;width:16px;"  onclick="alert(\'help\');" >
              ...
              <div id="'.$this->strControlId.'content1"  >
              '.$strToReturn.'
              </div>
...
$strToReturn = str_replace('{{MENU}}', '', $strToReturn);    
}
    ...
    protected function btnRefresh_Create($id) {
        $btnRefresh = $this->GetChildControl($id);
        if (!$btnRefresh) {
            $btnRefresh = new QButton($this, $id);
            $btnRefresh->Display = false;
            $btnRefresh->AddAction(new QClickEvent(), new QAjaxControlAction($this, 'btnRefresh_Click', $this->objWaitIcon));
            $btnRefresh->AddAction(new QClickEvent(), new QTerminateAction());
        }
        return $btnRefresh;
    }
