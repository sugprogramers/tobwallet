...
/*				return array();
		...
		public function SetCustomAttribute($strName, $strValue) {
			...
			if (!is_null($strValue))
                ...
                public function SetCustomAttributes($arrayAttributes){
                        $this->blnModified = true;
                        if(!empty($arrayAttributes)){
                            foreach ($arrayAttributes as $name => $value) {
                                if (!is_null($value))
                                    $this->strCustomAttributeArray[$name] = $value;
                                else {
                                        $this->strCustomAttributeArray[$name] = null;
                                        unset($this->strCustomAttributeArray[$name]);
                                }
                            }
                        }
...
//					$strOutput = sprintf('<ins id="%s_ctl" style="%stext-decoration:inherit;">%s</ins>%s', $this->strControlId, $strStyle, $strOutput, $this->GetNonWrappedHtml());
					...
					break;
                       ...
                       // $strOutput= htmlspecialchars($strOutput);
			// Output or Return
                        
                        
                        
                        /*$strOutput = sprintf('<script type="text/javascript">window.onload = function() { document.write("%s");}; </script>', $strOutput);
			*/
                        if ($blnDisplayOutput)
				print($strOutput);
			else
			...
			try {
				...
				if ($this->strValidationError)
				   	//$strOutput .= sprintf('&nbsp;<span class="warning"  >%s</span>', $this->strValidationError);
                                        $strOutput .= '<script>document.getElementById("'.$this->ControlId.'").style.borderColor = "#f96868";</script>';//.sprintf('<div style="color:#f96868;font-size:11px;text-align:left;margin: 0;"  >%s</div>', $this->strValidationError);
