		...
		protected function makeJsProperty($strProp, $strKey) {
			...
			if (null === $objValue) {
				return '';
                        ...
                        if($strKey == 'source') //Codigo agregado
                        {
                            //$objValue->strBody = " if(request.term.indexOf('%') == -1) return; " . $objValue->strBody;
                            $objValue->strBody = " if( request.term.charAt(request.term.length-1)  !='%') return; " . $objValue->strBody;
                            
                            //$objValue->strBody = " if(event.keyCode != 37) return; " . $objValue->strBody;
                            //$objValue->strBody = " if(true) test(); " . $objValue->strBody;
                        }
