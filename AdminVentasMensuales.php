...
class AdminVentasDiarias extends QForm
{
    ...
    protected function Form_Create()
    {
        ...
        $query = "SELECT DATE_FORMAT(FechaVenta,'%M %Y') as mes, 
                  ...
                  where month(FechaVenta) <= month( DATE_ADD(now(), INTERVAL 1 DAY)) and 
        ...
        $this->btnFilter->AddAction(new QClickEvent(), new QServerAction('actionFilter_Click'));
    ...
    public function actionFilter_Click($dini = null, $dfin = null)
    {
        ...
        if (trim($this->lstSucursal->SelectedValue != 0)) {
            $query ="   SELECT DATE_FORMAT(FechaVenta,'%M %Y') as mes, sucursal.Nombre as suc_nombre, sum(venta.TotalVenta) as TotalVenta,
                        ...
                        FROM venta
                                inner join sucursal on sucursal.IdSucursal = venta.IdSucursal
                        where 	sucursal.IdSucursal= $idSuc  and month(FechaVenta) <= $mes_end and 
            ...
            $query ="   SELECT DATE_FORMAT(FechaVenta,'%M %Y') as mes, sucursal.Nombre as suc_nombre, sum(venta.TotalVenta) as TotalVenta,
                        ...
                        FROM venta
                                inner join sucursal on sucursal.IdSucursal = venta.IdSucursal
                        where 	month(FechaVenta) <= $mes_end and 
                                month(FechaVenta) >= $mes_start
                                group by month(venta.FechaVenta)";
